...
4. **If anything reached `main` this session, write the changelog entry** — add it to `src/lib/changelog/entries.ts` (newest first), then commit and push it with everything else. It is what the team sees in the "What's new" popup, so:
   ...
   - `time` is 24-hour `HH:MM` Singapore time; the popup formats it to AM/PM. **Take it from the Vercel PRODUCTION deployment for that release** — open the deployment and read its created time (it shows GMT+8). That is the moment the team could actually see the change, which is what they read the timestamp as. (Nic, 2026-09-09.)
     - **Never take the time from this machine's clock — not `TZ=Asia/Singapore date`, and not plain `date` either.** `TZ=…` is ignored (no tzdata here) and **silently returns UTC** — that put `07:33` on a release that went live at `15:24`. Plain `date` is no safer: this shell reports itself as **MPST**, not SGT (Nic, 2026-09-10). Neither is Singapore time, and neither is the deployment's time.
     - **Decode it from the production deployment instead** — that is Vercel's own clock, and it is the moment the team could actually see the change. If the dashboard is not to hand, read it off a production response: `curl -sI https://greenqubes-ops.vercel.app/login | grep -i x-vercel-id` returns `…::<id>-<epoch-ms>-<hash>`; take the epoch-ms, drop the last 3 digits, and convert to UTC+8 with `date -u -d @$((TS + 8*3600)) '+%Y-%m-%d %H:%M'`. Probe **after** the deploy has landed, not before, or you stamp the previous release. Sanity-check the result: a wrong answer is usually 8 hours out.
