...
- [x] **[Nic] V3 working folder REMOVED — DONE 2026-09-07.** `C:\Greenqubes_GitHub\greenqubes-ops-workflow-v3` is off this PC. Before it went, Claude confirmed it held nothing unsaved: no uncommitted edits, and every commit already on GitHub. The `feat-workflow-v3` branch itself is untouched and still on GitHub as the permanent record.
- [x] **[Nic] Cancellation recorded** — `feat-workflow-v3` is now a historical-record branch like `feat-workflow-v2`: kept forever, never pushed to again. The design spec and both build plans carry a "cancelled — do not build from this" banner so no future session picks them up. If project containers ever come back, it starts with a fresh design session.
...
- [x] **[Nic] V3 folder removed from this PC (your call).** Claude verified first that it held no unsaved work and that everything was already on GitHub. The branch itself is untouched and kept as the permanent record.
