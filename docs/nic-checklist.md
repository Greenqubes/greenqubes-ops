...
- [ ] **Exclude Sales, Designer and Coordinator from the Support crew list** — Nic, 2026-09-07. Worth a moment's thought first: that bucket was deliberately widened to ALL roles on 2026-09-04 so anyone could be dispatched for a night job or a manpower shortage, and this partly reverses that. Everyone else stays (installers, production, scheduler, admin).
- [ ] **⚠️ STILL MISSPELLED — "Electrcian".** Nic said on 2026-09-07 that he had fixed this, but a live check at session end found it unchanged: 1 person still reads `Electrcian`, and 0 read `Electrician`. Either the edit did not save or a different person was edited. **Re-check in Admin → Users**, and if it looks right on screen but the check still fails, say so — a save that reports success without writing is exactly the class of bug this session fixed three times, and the admin user form has not been audited for it. A pill labelled Electrician matches nobody until this is right.
- [ ] **"Carpentry" is not one value, it is three** — the live data has `Carpenter` (3), `Senior Carpenter` (1) and `Assistant Carpenter` (1). Decide: one Carpentry pill that matches all three (recommended, no data change), or tidy the subroles down to one value. Same question in miniature for `Metalwork` (4) — Nic wrote "metalworks", the data says `Metalwork`.
- [ ] **The seven requested filters are a MIX of roles and subroles, not all roles** — `installer` and `production` are roles; `metalworks`, `carpentry`, `electrician`, `painter` and `printing` are subroles, and every one of them sits under the production role. Installers currently have NO subroles at all (all 7 blank), so the Installer pill can only filter by role. The pill row therefore has to match on either field.
...
- [ ] **Add Support crew AND External installers to the NEW job form** — confirmed missing: the new-job form has neither, while the edit form has both. So a job can only get support crew or an outside installer after it has been created and reopened.
...
- [ ] **Say go and the build starts at step 1.** Nothing else is waiting on you first. Roughly: the new role and its menu, the Leave page, the leave-clash warnings, the schedule markers, then your test round.
- [ ] **Approve the database change when the build reaches it** — two additions (the new role name, then the leave + public-holiday tables). They must go in **before** the code goes live, which is the usual order, and they're additive: nothing existing changes or breaks while they sit there unused. Claude will ask you to run `npx supabase db push` at the right moment.
- [ ] **Check the 2026 Singapore public holidays on the preview** — Claude pre-fills the 11 gazetted dates, and you or HR confirm them against the MOM website before it goes live. After that HR adds each new year herself from the Leave page (about 11 entries, once a year) — there's no outside service involved.
- [ ] **Provision the HR account** when it ships (Admin → Users → role "HR / Finance"), then have her sign in and Connect Telegram like everyone else.
- [ ] **One thing to be aware of, not a task:** she will see **every job's prices** — that's deliberate, since she's also the finance role — but she can only look, not change them. Sales keeps entering the figures. And the *reason* someone is on leave (annual / medical / emergency) is locked to her and the admin at the database level; everyone else only ever sees "On leave".
- [ ] **Deliberately left out, for a future session if you want them:** leave balances (days used vs entitlement), automatic entitlement counting, and staff requesting leave themselves for HR to approve. Today HR simply records what's already been agreed.
