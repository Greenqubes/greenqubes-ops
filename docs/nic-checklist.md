...
- [ ] **Check the 11 Singapore public holidays** against the MOM website — they're live on the schedule now, so the team can see them. The moving ones are worth a look: Chinese New Year (17–18 Feb) and Hari Raya Puasa (21 Mar). Edit any of them straight from the Leave page.
- [ ] **Provision the HR account** — Admin → Users → role "HR / Finance", then have her sign in and Connect Telegram like everyone else. She gets the guided tour on her first sign-in.
...
### Remaining checks from the test run (from 2026-09-09)

_Your tick-through page: https://claude.ai/code/artifact/68d57154-b1a7-44b2-bb40-50d7f57ad032 — it remembers what you tick._

- [ ] **The Telegram alerts** — put someone on a scheduled job, then record leave covering it. The scheduler, person-in-charge and coordinators should all get a bell alert and a Telegram with a link to the job. Someone with no jobs should ring the bell only, no Telegram.
- [ ] **The assistant** — ask it whether someone can take a job on a day they're on leave. It should mention the leave and never the reason.
- [ ] **The HR tour** — needs the HR account provisioned and a fresh sign-in. Also worth collecting Chinese and Bengali corrections from the team while you're there; both are unvetted.
- [ ] **The half-day case, on production** — someone on morning-only leave put on a 2pm job should produce **no warning anywhere**. It's the one test that proves half days work, and the easiest to miss because it asserts that nothing happens.
