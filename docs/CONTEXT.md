...
- [x] **Workflow V3 — project containers — ⛔ CANCELLED 2026-09-04 [Nic]**, archived unmerged. Round 1 was built + smoke-cleared 2026-09-02 on `feat-workflow-v3`; the demo launch then raised enough feedback and forthcoming changes to nullify the scope, so rounds 2–3 were dropped before any code and nothing was ever merged. Branch kept for historical record only (no new pushes). Migration 0051 stays applied to the shared DB — additive-only, no code reads it, and 0052/0053 build on its numbering. Spec + round-1 plan carry cancelled banners; the guided-tour "add Projects steps" follow-up is void. Smoke-test leftovers in the shared DB (2 projects, 8 buckets, 1 file) await Nic's cleanup call.
