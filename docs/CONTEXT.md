...
_Last updated: 2026-08-25 (feat-assistant-3 — assistant upgrade **Phases 1 + 2 + 3 live on production**, all shipped the same day: Sonnet 5 brain + Claude-grade UI (1); agentic tool loop, six RLS-scoped live-data tools, agentic KB search, per-user memory + Memory manager (2, migration 0046); chat attachments (images + PDF) → confirm-first `create_pending_job` with bucket auto-filing + link chip, Move-file-between-buckets on the job form, 30-day scratch cleanup cron (3, no migration). Only Phase 4 Projects remains — migration 0047 BEFORE its code push. Memory stays strictly per-user; installer-login + two-account isolation checks deferred to production after all phases.)_
