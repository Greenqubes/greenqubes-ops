...
_Last updated: 2026-08-25 (feat-assistant-2 — assistant upgrade **Phases 1 + 2 live on production**: Sonnet 5 brain + Claude-grade UI (Phase 1), agentic tool loop with six read-only RLS-scoped live-data tools, agentic KB search, per-user memory summaries + Memory manager (Phase 2, migration 0046 applied before code). Phases 3–4 pending; memory stays strictly per-user, digest→vault the only cross-user knowledge bridge. Installer-login + two-account isolation checks deferred to production after all phases.)_
