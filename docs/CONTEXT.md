...
- [ ] **Workflow V3 — project containers** — round 1 BUILT 2026-09-02 on `feat-workflow-v3` (NOT merged; migration 0051 applied; Nic's smoke round cleared with 5 feedback fixes). Merge only when fully built (Nic's rule): round 2 (schedule folding + filter chips replacing the Pending/Completed tabs + pending-privacy RLS + sort/filter dropdown) and round 3 (Notion-style month view + project pop-outs) remain. Spec `docs/superpowers/specs/2026-09-02-workflow-v3-nesting-design.md`; worktree kept at `../greenqubes-ops-workflow-v3`. **At the V3 merge: also add Projects steps to the guided-tour scripts.**
