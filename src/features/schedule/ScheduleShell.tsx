...
export function ScheduleShell({ jobs, lang, role, pageMode = 'schedule' }: ScheduleShellProps) {
  ...
  const [confirmBulk,  setConfirmBulk]  = useState<'delete' | 'revert' | false>(false)

  const canBulkDelete = role === 'scheduler' || (role === 'sales' && pageMode === 'pending')
  // Undo accidental completions straight from the Completed list (Nic, 2026-08-19).
          ...
          {confirmBulk ? (
            <>
              ...
              <div className="flex gap-2 shrink-0">
                ...
                <button
                  onClick={confirmBulk === 'delete' ? handleBulkDelete : handleBulkRevert}
                  disabled={bulkBusy}
                  className="px-3 py-1.5 text-xs font-medium text-white rounded-lg transition-colors"
                  style={{ backgroundColor: 'var(--terracotta)' }}
                >
                  {bulkBusy
                    ? (confirmBulk === 'delete' ? 'Deleting…' : 'Reverting…')
                    : 'Confirm'}
                </button>
              </div>
            </>
