...
import { Bell, X, AlertCircle, ArrowRight } from 'lucide-react'
    ...
    <>
      ...
      <button
        ...
        className={cn(
          ...
          count > 0
            ? 'bg-brand-amber-soft border-brand-amber text-brand-amber'
            : 'bg-paper border-line text-ink2 hover:border-ink2',
        )}
      >
        <Bell size={15} />
        {count > 0 && (
          <span className="absolute -top-1.5 -right-1.5 min-w-[16px] h-4 flex items-center justify-center bg-brand-amber text-white text-[10px] font-bold rounded-full px-1 leading-none">
            {count}
          </span>
        )}
      </button>
            ...
            <div className="px-3 py-3 space-y-2">
              ...
              {overdueJobs.map(job => (
                <Link
                  ...
                  href={`/jobs/${job.id}`}
                  onClick={() => setOpen(false)}
                  className="flex items-start gap-2.5 p-3 rounded-xl border border-brand-amber/40 bg-brand-amber-soft hover:border-brand-amber transition-colors group"
                >
                  <AlertCircle size={14} className="text-brand-amber mt-0.5 shrink-0" />
                  <div className="flex-1 min-w-0">
                    <p className="text-xs font-medium text-ink truncate">{job.client}</p>
                    <p className="text-[11px] text-ink2 mt-0.5">{job.date}</p>
                    {job.location && (
                      <p className="text-[11px] text-muted truncate">{job.location}</p>
                    )}
                  </div>
                  <ArrowRight
                    size={12}
